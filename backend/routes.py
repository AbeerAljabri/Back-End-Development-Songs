...
@app.route("/count", methods=["GET"])  # Replace "{insert url here}" with "/count"
def count():
    """Return length of data"""
    count = db["songs"].count_documents({})  # Uses count_documents to count all documents
    return jsonify({"count": count}), 200  # HTTP 200 OK

@app.route("/song", methods=["GET"])  
def songs():
    """Return the data"""
    song_list =  list(db["songs"].find({}))   # Uses count_documents to count all documents
